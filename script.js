...
let score = 0;
let bestScore = Number(localStorage.getItem('infiniteStairsBest')) || 0;
let bestScoreAtStart = bestScore; // 게임 시작 시점의 최고기록 스냅샷
...
const MAX_DEPLETION_RATE = 22; // maximum speed depletion rate cap
const MAX_STAIRS_AHEAD = 20;
const MAX_STRAIGHT = 4; // maximum consecutive stairs in same direction
...
function initGame() {
    ...
    // 첫 번째 계단이 있는 방향으로 캐릭터 방향 설정
    if (stairs.length >= 2) {
        playerDir = stairs[1].x > stairs[0].x ? 1 : -1;
    }
...
function generateStair() {
    ...
    if (stairs.length >= 2) {
        ...
        // Count consecutive stairs in the same direction
        let streak = 1;
        for (let i = stairs.length - 2; i >= 1; i--) {
            const d = stairs[i].x > stairs[i - 1].x ? 1 : -1;
            if (d === prevDir) streak++;
            else break;
        }
        
        // Force a turn if streak reached the maximum, otherwise 65% same / 35% turn
        if (streak >= MAX_STRAIGHT || Math.random() < 0.35) {
            dir = prevDir * -1;
        } else {
            dir = prevDir;
        }
...
function update(deltaTime) {
    ...
    if (score > 0) { // Speed only depletes after first step
        // Depletion rate increases with score, capped at MAX_DEPLETION_RATE
        const currentRate = Math.min(BASE_DEPLETION_RATE + (score * 0.05), MAX_DEPLETION_RATE);
...
function triggerGameOver(reason) {
    ...
    setTimeout(() => {
        ...
        // localStorage에서 직접 읽어 비교 (변수 상태 오염 방지)
        const rawStored = localStorage.getItem('infiniteStairsBest');
        const storedBest = rawStored !== null ? parseInt(rawStored, 10) : 0;
        const validStoredBest = isNaN(storedBest) ? 0 : storedBest;
        
        if (score > 0 && score > validStoredBest) {
            bestScore = score;
            localStorage.setItem('infiniteStairsBest', String(score));
